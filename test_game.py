...
def test_t04():
    ...
    check("T04", "消除本关全部箭头",
          cleared and game.state == main.GameState.LEVEL_COMPLETE, "显示通关")
    game.load_level(game.current_level + 1)
    check("T05", "通关后进入下一关",
          game.state == main.GameState.PLAYING and game.current_level == 1, "进入关卡 2")
...
def test_t07():
    ...
    game.handle_click(arrow.get_rect().center)
    check("T07", "失误次数耗尽",
          game.state == main.GameState.GAME_OVER and game.mistakes == main.LEVELS[2].max_mistakes,
          "显示失败")
...
def test_t10():
    ...
    check("T10", f"全部 {len(main.LEVELS)} 个关卡都存在通关顺序", all(solved), "逐关贪心点击验证")


if __name__ == "__main__":
    tests = (test_t01, test_t02, test_t03, test_t04, test_t06,
